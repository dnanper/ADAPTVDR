...
class Qwen3VLForEmbedding(Qwen3VLPreTrainedModel):
    ...
    def _get_decoder_layers(self):
        ...
        raise AttributeError("Could not locate decoder layers on Qwen3-VL model")

    def _forward_single_layer_attention(
        self,
        ...
        **model_kwargs,
        ...
        target_index = attention_layer_index if attention_layer_index >= 0 else num_layers + attention_layer_index
        if target_index < 0 or target_index >= num_layers:
            raise IndexError(
                f"attention_layer_index={attention_layer_index} is out of range for {num_layers} decoder layers"
            )

        target_layer = decoder_layers[target_index]
        original_forward = target_layer.self_attn.forward
        captured_attention = {}

        def wrapped_forward(*args, **kwargs):
            layer_outputs = original_forward(*args, **kwargs)
            if not isinstance(layer_outputs, tuple) or len(layer_outputs) < 2:
                raise ValueError("Target self-attention did not return attention weights")
            captured_attention["tensor"] = layer_outputs[1]
            return layer_outputs

        target_layer.self_attn.forward = wrapped_forward
        try:
            outputs = self.model(
        ...
        attention = captured_attention.get("tensor")
        if attention is None:
            raise ValueError("Failed to capture attention weights from the target layer")
    ...
    def forward(self,
                ...
                attention_layer_index: Optional[int] = None,
        ...
        if output_attentions and attention_layer_index is not None:
            outputs, attentions = self._forward_single_layer_attention(
